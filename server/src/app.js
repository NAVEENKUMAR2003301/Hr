...
// Split from the `.listen()` call (see index.js for local dev, api/index.js for
// Vercel) so the same Express app can be either a normal long-running server or
// wrapped as a single serverless function — Vercel's Node runtime accepts an
// Express app directly as a request handler.
const app = express();

// Multiple origins allowed (comma-separated CLIENT_URL) so both a local dev
// frontend and the deployed Vercel frontend can hit the same backend during
// the transition, instead of hardcoding exactly one.
...
// that would let anyone with the URL download a file unauthenticated. Only used for
// documents still on local disk (pre-Cloudinary-migration, or in local dev); new
// uploads get a signed Cloudinary URL instead — see documents controller.
